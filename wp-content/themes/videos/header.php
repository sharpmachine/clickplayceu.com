...
<head profile="http://gmpg.org/xfn/11">
	...
	<script type="text/javascript">
		var affidavit_signed = <?php echo json_encode((bool) get_user_meta(wp_get_current_user()->ID, 'affidavit', true)); ?>;
				...
				var state = jQuery('#region_select_35 select').val();
			     ...
			     if ((profession == "arf") || (profession == "gh") || (profession == "rfe") || (profession == "cna") ) {
			      ...
			      if (profession == "rn" ) {
			      	...
			      	jQuery('#affidavit_signed').attr('name', 'collected_data[49][]');
			      } else {
			      	jQuery('tr.wpsc_checkout_form_31').hide();
			      	jQuery('#affidavit_signed').attr('name', affidavit_signed ? 'collected_data[49][]' : '');
			      }
		    });
		...
		function updateAffidavitLink(url) {
			console.log(url);
			var link_element = document.getElementById('affidavit');
			if (url) {
				link_element.innerHTML = 'View Signed Affidavit';
				link_element.href = <?php echo json_encode(content_url() . '/uploads/signatures/'); ?> + url;
