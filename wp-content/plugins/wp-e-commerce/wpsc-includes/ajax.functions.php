...
if (isset($_GET['affidavit']) && ($_GET['affidavit']) === 'true') {
ini_set('display_errors',1);
error_reporting(E_ALL|E_STRICT);
	global $wpdb;
	$current_user = wp_get_current_user();
	$filename = $_GET['filename'];
	update_user_meta( $current_user->ID, 'affidavit', $filename );
	//$uploads = wp_upload_dir(); 
	//$move_to = $uploads['path']."\\signatures\\".$filename;
	$move_to = WP_CONTENT_DIR . "/uploads/signatures/".$filename;
	$move_from = WP_CONTENT_DIR."/plugins/wp-e-commerce/wpsc-includes/signature/".$filename;
	rename($move_from, $move_to);
	//cleanup
	// suppress errors, they should already be deleted
	@unlink($move_from);
	@unlink("withtext.png");
	@unlink($targetfile);
	@unlink($logo_file);
	echo '<head><title></title><script language="javascript" type="text/javascript">parent.updateAffidavitLink(' . json_encode($filename) . ');</script></head><body><h2>Done, you may now close this popup</h2></body>';
	exit();
}

if ( isset( $_GET['signaffidavit'] ) && ($_GET['signaffidavit'] === 'true') ) {
	...
	$document .= "<head><title>Sample - PDF Document Signing</title>";
	$document .= "				<script language=\"javascript\" type=\"text/javascript\">";
	$document .= "					document.oncontextmenu = new Function(\"return false;\");";
	$document .= "				</script>";
	$document .= "				<link rel=\"stylesheet\" type=\"text/css\" href=\"".plugins_url()."/wp-e-commerce/wpsc-includes/signature/styles.css\" />";
	$document .= "				<script type=\"text/javascript\" src=\"".plugins_url()."/wp-e-commerce/wpsc-includes/signature/wz_jsgraphics.js\"></script> <!-- if you need windows mobile support -->";
	$document .= "				<script type=\"text/javascript\" src=\"".plugins_url()."/wp-e-commerce/wpsc-includes/signature/ss.js\"></script>";
	$document .= "				</head>";
	$document .= "				<body>";
	$document .= "				    <form method=\"post\" action=\"".plugins_url()."/wp-e-commerce/wpsc-includes/signature/confirmation.php\">";
	$document .= "				    <center><img src=\"".plugins_url()."/wp-e-commerce/wpsc-includes/signature/affidavit-20111203.gif\" />";
	$document .= "						<div onclick=\"ShowSignModal();\" id=\"divSign\">";
	$document .= "							<b>&nbsp;&nbsp;Click To Sign</b>";
	$document .= "					    </div>";
	$document .= "					</center>";
	$document .= "					<table id=\"overlay\">";
	$document .= "						<tr>";
	$document .= "							<td>";
	$document .= "								<center>";
	$document .= "								<div id='ctlSignature_Container' style='width:400px;height:270px;margin:20px'>";
					$msie = strpos($_SERVER["HTTP_USER_AGENT"], 'MSIE') ? true : false; 
					if ($msie == true) {
	$document .= "		           	<div ID='ctlSignature' style='width:400px;height:250px;border:Dashed 2px #DDDDDD'></div>";
					} else {
	$document .= "		    		<canvas ID='ctlSignature' width='400' height='250'></canvas>";
			  		}						    
	$document .= "							    </div>";
	$document .= "				                </center>";
	$document .= "				              </td>";
	...
	$document .= "				              <td>";
	$document .= "				              	<center>";
	$document .= "				              		<input type=\"submit\" name=\"btnSave\" value=\"Save\" onclick=\"javascript:return ValidateSignature();\" id=\"btnSave\" class=\"save\" />&nbsp;";
	$document .= "				                	<input type=\"button\" id=\"btnCancel\" class=\"cancel\" onclick=\"ShowSignModal();\" value=\"Cancel\" />";
	$document .= "				                </center>";
	$document .= "				              </td>";
	$document .= "				            </tr>";
	$document .= "				         </table>";
	$document .= "						<input type=\"hidden\" name=\"firstname\" value=\"". $current_user->first_name ."\">";
	$document .= "						<input type=\"hidden\" name=\"lastname\" value=\"". $current_user->last_name ."\">";
	$document .= "						<input type=\"hidden\" name=\"userid\" value=\"". $current_user->ID ."\">";
	$document .= "						<input type=\"hidden\" name=\"email\" value=\"". $current_user->user_email ."\">";
	$document .= "				</form>";
	$document .= "				<script type=\"text/javascript\">";
	$document .= "					var signObjects = new Array('ctlSignature');";
	$document .= "					var objctlSignature = new SuperSignature({SignObject:\"ctlSignature\",SignWidth: \"400\",SignHeight: \"250\",PenColor: \"#000000\",BorderStyle: \"Dashed\",BorderWidth: \"2px\",BorderColor: \"#DDDDDD\",RequiredPoints: \"25\",ClearImage:\"".plugins_url()."/wp-e-commerce/wpsc-includes/signature/refresh.png\", PenCursor:\"".plugins_url()."/wp-e-commerce/wpsc-includes/pencil.cur\", Visible: \"true\"});";		
	$document .= "					objctlSignature.Init();";
	$document .= "				   </script>";
	$document .= "				    <script language=\"javascript\">";
	$document .= "				        function ShowSignModal() {";
	$document .= "				            el = document.getElementById(\"overlay\");";
	$document .= "				            el.style.visibility = (el.style.visibility == \"visible\") ? \"hidden\" : \"visible\";";
	$document .= "				            ";
	$document .= "				            el2 = document.getElementById(\"divSign\");";
	$document .= "				            el2.style.display = (el2.style.display == \"\") ? \"none\" : \"\";";
	$document .= "				";
	$document .= "				            if (el.style.visibility == \"visible\") {";
	$document .= "				                document.body.style.backgroundColor = \"#DDDDDD\";";
	$document .= "				                document.getElementById(\"btnSave\").focus();";
	$document .= "				            }";
	$document .= "				            else";
	$document .= "				            {";
	$document .= "				                document.body.style.backgroundColor = \"#FFFFFF\";";
	$document .= "				            }";
	$document .= "				        }";
	$document .= "				    </script>";
	$document .= "				</body>";
