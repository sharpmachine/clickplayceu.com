			...
			case "select":
				...
				if ( $options != '' ) {
					...
					foreach ( (array)$options as $label => $value ) {
						...
						// make exception for Profession field (ID 36)
						$output .="<option " . ($this->checkout_item->id != 36 ? selected( $value, $saved_form_data, false ) : '') . " value='" . esc_attr( $value ) . "'>" . esc_html( $label ) . "</option>\n\r";
					}
					$output .="</select>";
				}
				break;
