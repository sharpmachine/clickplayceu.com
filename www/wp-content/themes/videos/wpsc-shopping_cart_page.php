  ...
  <?php if(wpsc_uses_shipping()): ?>
   ...
   <?php do_action('wpsc_before_form_of_shopping_cart'); ?>
	...
	<?php if ( wpsc_show_user_login_form() && !is_user_logged_in() ): ?>
			<p>You must be logged in to complete the transaction.  <a href="<?php bloginfo('url'); ?>/products-page/your-account/">Log in here.</a></p>
            ...
            <?php
               ...
               "<div class='wpsc_email_address'>
                  <p class='<?php echo wpsc_checkout_form_element_id(); ?>'>
                     ...
                     " . __('<strong>First, please enter the email address you want the certificate sent to</strong>', 'wpsc') . "
                     </label>
               ...
               $email_markup .= "</div>";
             	...
             	if (wpsc_checkout_form_element_id() == 'wpsc_checkout_form_37'): ?>	
		            <tr class='<?php echo wpsc_checkout_form_element_id(); ?>'>
			            <td colspan='2'>
			            	<?php if (is_user_logged_in())  {
		            			$current_user = wp_get_current_user();
		            			//echo 'Filename:' . get_user_meta($current_user->ID, 'affidavit', true);
		            			if (get_user_meta($current_user->ID, 'affidavit', true)) {
		            				printf(__("<a target='_blank' href='%s' id='affidavit' class='Affidavit'>View Signed Affidavit</a>", "wpsc"),  content_url() . "/uploads/signatures/".get_user_meta($current_user->ID, 'affidavit', true)); 
		            			} else {
		            				printf(__("<a target='_blank' href='%s' id='affidavit' class='Affidavit'>Please Sign Affidavit </a>", "wpsc"),  site_url("?signaffidavit=true'"));
		            			}
			            	} else {
		            		 	printf(__("<a target='_blank' href='%s' id='affidavit' class='Affidavit'>Please Sign Affidavit</a>", "wpsc"),  site_url("?signaffidavit=true'"));
			            	} ?>
		         ...
		         <?php elseif (wpsc_checkout_form_element_id() == 'wpsc_checkout_form_49') :?>
					
					<input type="hidden" id="affidavit_signed" name="<?php echo (get_user_meta($current_user->ID, 'affidavit', true)) ? 'collected_data[49][]' : ''; ?>" value="present" checked />
