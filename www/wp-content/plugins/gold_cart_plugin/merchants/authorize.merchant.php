  ...
  var $credit_card_details = array(
	...
	*/
	function construct_value_array() {
		...
		/*
		$h=fopen('file.txt','w+');fwrite($h,print_r($this->cart_data['total_price'], true));fclose($h);
...
// 	  	exit();
		...
		/*
			Can't process a $0 transaction, mock the API response as successful
			because no transaction is required.
		*/
		if (intval($this->collected_gateway_data['total_price']) > 0) {
		...
		//$h=fopen('file2.txt','w+');fwrite($h,print_r($parsed_response, true));fclose($h);
		//echo "<pre>";
