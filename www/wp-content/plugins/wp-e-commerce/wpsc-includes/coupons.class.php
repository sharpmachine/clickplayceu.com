...
class wpsc_coupons {
	...
	function wpsc_coupons($code = ''){
		...
		if ($code == '') {
			...
			if (($coupon_data == '') || ($coupon_data == null) || (strtotime($coupon_data['expiry']) < time()) || ($coupon_data['uses_remaining'] == 0)) {
				$this->errormsg = false;
				return false;
			} else {
				...
				$this->uses_remaining = $coupon_data['uses_remaining'];
	...
	function validate_coupon() {
		...
		if ( ($this->active=='1') && !(($this->use_once == '1') && ($this->is_used=='1')) && ($this->uses_remaining > 0)){
