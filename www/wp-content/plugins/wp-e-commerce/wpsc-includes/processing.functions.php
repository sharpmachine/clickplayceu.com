 ...
 * @return int of productid
 */
function wpsc_get_product_id($purchase_id) {
	global $wpdb; 
	$productid = $wpdb->get_var("SELECT `prodid` FROM `".WPSC_TABLE_CART_CONTENTS."` WHERE `purchaseid` ='". $purchase_id. "' LIMIT 1"); 
	return $productid;
}

function wpsc_get_credit_count($purchaseid) {
	global $wpdb;
	$totalcredits = 0;
	$purchases = $wpdb->get_results("SELECT * FROM `" . WPSC_TABLE_CART_CONTENTS . "` WHERE `purchaseid` = '".$purchaseid."'");
	if ($purchases) {
		foreach($purchases as $product) {
			$credits = get_post_meta($product->prodid, 'Credits', True);
