...
function wpsc_display_coupons_page() {
	...
	if ( isset( $_POST ) && is_array( $_POST ) && !empty( $_POST ) ) {
		...
		if ( isset( $_POST['add_coupon'] ) && ($_POST['add_coupon'] == 'true') && (!isset( $_POST['is_edit_coupon'] ) || !($_POST['is_edit_coupon'] == 'true')) ) {
			...
			$uses_remaining= (int)$_POST['uses_remaining'];
			...
			if ( $wpdb->query( "INSERT INTO `" . WPSC_TABLE_COUPON_CODES . "` ( `coupon_code` , `value` , `is-percentage` , `use-once` , `is-used` , `active` , `every_product` , `uses_remaining`, `start` , `expiry`, `condition` ) VALUES ( '$coupon_code', '$discount', '$discount_type', '$use_once', '0', '$is_active', '$every_product', '$uses_remaining', '$start_date' , '$end_date' , '" . serialize( $new_rule ) . "' );" ) )
		...
		if ( isset( $_POST['is_edit_coupon'] ) && ($_POST['is_edit_coupon'] == 'true') && !(isset( $_POST['delete_condition'] )) && !(isset( $_POST['submit_condition'] )) ) {
			...
			foreach ( (array)$_POST['edit_coupon'] as $coupon_id => $coupon_data ) {
				...
				$check_values          = $wpdb->get_row( "SELECT `id`, `coupon_code`, `value`, `is-percentage`, `uses_remaining`,`use-once`, `active`, `start`, `expiry`,`every_product` FROM `" . WPSC_TABLE_COUPON_CODES . "` WHERE `id` = '$coupon_id'", ARRAY_A );
						...
						<form name='add_coupon' method='post' action=''>
							<table class='add-coupon' >
								...
								<tr>
									...
									<td>
										<input type='text' value='' name='uses_remaining' />
									</td>
									<td>
			...
			<tbody>
				<?php
					...
					foreach ( (array)$coupon_data as $coupon ) {
						...
						echo "    <td>\n\r";
						esc_attr_e( $coupon['uses_remaining'] );
						echo "    </td>\n\r";
